...
module.exports = parameters => {
  ...
  const parallel = async (jobs, search) => {
    await Promise.all(jobs.map(json => {
  ...
  const stop = async (ms = 5000) => {
    let forcedStop = setTimeout(() => {
      throw new Error("Scheduler stopping timeout");
    }, ms);
    this.stopping = true;
  ...
  return {
    async start() {
      ...
      client.on('error', async (error) => {
        await stop();
        console.error('Redis Client error, closed connection', error);
      });
